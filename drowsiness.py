    ...
    while st.session_state.detection_running:
        ...
        for (x, y, w, h) in faces:
            ...
            if len(left_eye) > 0:
                ...
                l_eye_resized = cv2.resize(l_eye, (24, 24)) / 255.0
                l_eye_reshaped = l_eye_resized.reshape(1, 24, 24, 1)
            ...
            if len(right_eye) > 0:
                ...
                r_eye_resized = cv2.resize(r_eye, (24, 24)) / 255.0
                r_eye_reshaped = r_eye_resized.reshape(1, 24, 24, 1)
                rpred = np.argmax(model.predict(r_eye_reshaped), axis=1)[0]

            # Update score based on predictions
            if rpred == 0 or lpred == 0:
                score += 1
            else:
                score = max(0, score - 1)

            cv2.putText(small_frame, f'Score: {score}', (10, 30), font, 1, (255, 0, 0), 2)
...
choice=st.sidebar.selectbox("Video Source",["📷 Camera","🔗 URL","🎥 Video"],index=None,placeholder="Helo")
...
if choice=="📷 Camera":
    ...
    camera_choice = st.sidebar.selectbox("Select Camera Type", ["Primary", "Secondary", "IP Camera"], index=0)
